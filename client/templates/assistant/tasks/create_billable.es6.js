...
let compressStatuses = (statuses, fromTs, toTs, barHeight) => {
  ...
  for (let i = 1; i < result.length; i++) {
    if (result[i].time - result[i-1].time < minTimeDiff) {
      ...
      result = _.filter(result, function(r) { return !_.isEqual(r, result[i]); });
      i--;
    }
  }
...
Template.assistantTasksCreateBillable.onRendered(function() {
  ...
  let bar = data
      .append('rect')
      .attr('x', 55)
      .attr('y', function(d) { return ts(d.time) + barHeadPadding; })
  ...
  let line = data
          ...
          .attr('x1', 55)
          .attr('x2', 85)
          .attr('y1', function(d) { return ts(d.time) + barHeadPadding; })
          .attr('y2', function(d) { return ts(d.time) + barHeadPadding; })
          .attr('class', function(d) { return d.message ? "" : "hide" })
      ;
  ...
  messages
      ...
      .attr('y', function(d) { return ts(d.time) + barHeadPadding + 5; })
  ;
  ...
  let userName =
      data.append('text')
          ...
          .attr('x', 55 + thickness * 2 )
          .attr('y', function(d) { return ts(d.time) + barHeadPadding + 22; })
  ;
