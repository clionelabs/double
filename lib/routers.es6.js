...
// Wrapping the routers around Meteor.startup to avoid
// a mysterious error: In production mode, the `let xxx = yyy`
// statement on top will cause an 'Haneler name `b` already
// exists' error, which looks like a bug on compression
  ...
  Router.route('assistant/customers/:_id?', {
    ...
    waitOn() {
      return [
        ...
        Meteor.subscribe("assistants"),
        Meteor.subscribe("placements"),
        Meteor.subscribe("myInProcessTasks"),
