  ...
  let stateMachine = StateMachine.create({
    ...
    callbacks: {
      ...
      ontransactionSuccess: function(event, from, to) {
        ...
        //Email logic is not here because it is a server logic
...
Invoices.regenerateToken = (invoiceId) => {
  const newToken = Random.id();
  const expiredAt = moment().add(5, 'day').valueOf();
  return Invoices.update(invoiceId, { $set : { token : { value : newToken, expiredAt : expiredAt }}});
};