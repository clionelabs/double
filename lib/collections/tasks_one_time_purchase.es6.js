    ...
    let stateMachine = StateMachine.create({
      ...
      callbacks: {
        ...
        ontransactionSucceed(event, from, to) {
          //non server only method, seperate to method, observe logic is complicated
          Meteor.call('slackOtpTransactionSucceed', this);
        },
        ontransactionFailed(event, from, to) {
          //non server only method, seperate to method, observe logic is complicated
          Meteor.call('slackOtpTransactionFailed', this);
        },
        ontransactionVoided(event, from, to) {
          //non server only method, seperate to method, observe logic is complicated
          Meteor.call('slackOtpTransactionVoided', this);
        }
      }
    });
...
Tasks.OneTimePurchase.ProtoType = {
  ...
  totalAmount() {
    return (this.amount + this.extraCharge()).toFixed(2);
  }
};
