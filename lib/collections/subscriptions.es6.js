  ...
  ProtoType : {
    ...
    chargesFromPeriod(from, to) {
      ...
      while(!iDate.isAfter(tillMoment)) {
        const dateOfCharge = iDate.clone().subtract(1, 'day');
        const periodEndAt = iDate.clone().add(1, 'month').subtract(1, 'day');
        if (!dateOfCharge.isBefore(fromMoment)) {
          charges.push({
            chargedAt: dateOfCharge.valueOf(),
            periodEndAt : periodEndAt.valueOf(),
            amount: plan.amount
          });
        }
    ...
    plan() {
      if (this.planId === Plans.NoPlan._id) return Plans.NoPlan;
