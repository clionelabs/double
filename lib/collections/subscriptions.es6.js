...
Subscription = {
  ProtoType : {
    ...
    chargesFromPeriod(from, to) {
      ...
      const tillMoment = moment(_.min[to, this.endedAt]) + 1;
      const fromMoment = moment(from);
      while(!iDate.isAfter(tillMoment)) {
        let dateOfCharge = iDate.clone().subtract(1, 'day');
        if (dateOfCharge.isBetween(fromMoment, tillMoment, 'day')) {
...
Subscriptions.change = (newPlanId, customerId, cb) => {
  ...
  const currentDate = moment().startOf('day').add(1, 'day').valueOf();
