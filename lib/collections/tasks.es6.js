...
Task.Prototype = {
  ...
  totalDuration(from = 0, to = moment().endOf('day').valueOf() + 1) {
    return _.reduce(this.steps, function(memo, step) {
      return memo + step.duration(from, to);
    }, 0);
  },
  totalDurationInMinutes(from = 0, to = moment().endOf('day').valueOf() + 1) {
...
Task.Step = {};
Task.Step.ProtoType = {
  duration(startedAt = 0, endedAt = moment().endOf('day').valueOf() + 1) {
...
Tasks.OneTimePurchase = {
  add : (date, amount, title, isOnBehalf, taskId, cb) => {
    ...
    return Tasks.update ({ _id : taskId }, { $push : { otherCharges : oneTimePurchase }}, cb);
  },
  delete : (oneTimePurchaseId, taskId, cb) => {
    let oneTimePurchases = Tasks.findOne(taskId).otherCharges;
    oneTimePurchases = _.filter(oneTimePurchases, function(oneTimePurchase) {
      return oneTimePurchase._id !== oneTimePurchaseId;
    });
    return Tasks.update ({ _id : taskId }, { $set : { otherCharges : oneTimePurchases }}, cb);
  }
};

Tasks.Adjustments = {
  add: (reason, duration, taskId, cb) => {
    let adjustment = {
      _id : Random.id(),
      reason: reason,
      duration: duration,
    ...
    return Tasks.update({ _id : taskId }, { $push : { adjustments : adjustment }}, cb);
  },
  delete : (adjustmentId, taskId, cb) => {
    let adjustments = Tasks.findOne(taskId).adjustments;
    adjustments = _.filter(adjustments, function(adjustment) {
      return adjustment._id !== adjustmentId;
    });
    return Tasks.update ({ _id : taskId }, { $set : { adjustments : adjustments }}, cb);
  }
};
