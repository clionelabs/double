...
Tasks.editWork = function(taskId, timesheetIdToBeDeleted, startTime, endTime) {
  ...
  let removedTimesheetWithId = _.filter(timesheets, (timesheet) => {
  ...
  let newTimesheets = _.first(removedTimesheetWithId, whereToInsert)
                        .concat(newTimeSheet)
                        .concat(_.rest(removedTimesheetWithId, whereToInsert));

  return Tasks.update({ _id : taskId }, { $set : { timesheets : newTimesheets }});
};
