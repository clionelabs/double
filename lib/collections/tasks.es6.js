...
Tasks.findRequestedBy = (requestId, options={}) => {
  return Tasks.find(_.extend(options, { requestorId : requestId }));
};
...
Tasks._pushWork = (taskId, startedAt, endedAt) => {
  let newWork = {
    _id : Random.id(),
    startedAt: startedAt,
    endedAt: endedAt
...
Tasks.startWork = (taskId) => {
  let lastWork = Tasks._findLastWork(taskId);
 ...
 * @returns {any}
 */
Tasks.editWork = (taskId, timesheetIdToBeDeleted, startTime, endTime) => {
...
Tasks.Timesheet.Prototype = {
  //why not getter? `this` is not the extended object but Tasks.Timesheet.Prototype
  duration() {
    let work = this;
