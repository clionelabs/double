...
Tasks.Steps = {
  add : (itemText, taskId) => {
    ...
    Tasks.update({ _id : taskId }, { $push : { steps: step }});
    return step._id;
  },
  complete : (stepId, taskId) => {
    ...
    steps.push(_.extend(oldStep, { isCompleted : true }));
    steps = steps.concat(_.rest(task.steps, stepIndex));
    console.log(steps);
  }
};