...
Tasks.editTitle = (taskId, newTitle, cb) => {
  Tasks.update({ _id : taskId }, { $set : { title : newTitle }}, cb);
};
...
Tasks.Steps.Durations = {
  edit : (taskId, stepId, durationId, date, value, cb) => {
    ...
    Tasks.update({ _id : taskId }, { $set : { steps : task.steps }}, cb);
  },
...
Tasks.OneTimePurchase = {
  add : (date, amount, title,  taskId, cb) => {
    ...
    Tasks.update ({ _id : taskId }, { $push : { oneTimePurchases : oneTimePurchase }}, cb);
  },
  delete : (oneTimePurchaseId, taskId, cb) => {
    ...
    oneTimePurchases = _.filter(oneTimePurchases, function(oneTimePurchase) {
      oneTimePurchase._id !== oneTimePurchaseId;
    });
    Tasks.update ({ _id : taskId }, { $set : { oneTimePurchases : oneTimePurchases }}, cb);
  }
};
