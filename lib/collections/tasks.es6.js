...
Tasks.Steps = {
  ...
  toggleComplete : (taskId, stepId) => {
    let task = Tasks.findOne({ _id : taskId });
    let oldStep = _.find(task.steps, (step)=> {
      return step._id === stepId;
    });
    let stepIndex = _.indexOf(task.steps, oldStep);
    let steps = _.first(task.steps, stepIndex - 1);
    steps.push(_.extend(oldStep, { isCompleted : !oldStep.isCompleted }));
    steps = steps.concat(_.rest(task.steps, stepIndex + 1));
    Tasks.update({ _id : taskId }, { $set : { steps : steps }});
  },
