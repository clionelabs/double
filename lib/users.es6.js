...
Customers = {
  ...
  deductCreditMs : (userId, msToBeDeduct, from, to, cb) => {
    const selector = { _id : userId };
    const user = Users.findOneCustomer(selector);

    const subscriptionRatios = user.getSubscriptionRatio(user._id, from, to);
    _.each(_.sortBy(subscriptionRatios, 'date'), function(subscriptionRatio) {
      const plan = Plans.findOne(subscriptionRatio.planId);
      msToBeDeduct -= plan.freeCreditMs * subscriptionRatio.ratio;
    });
...
Customer = {
  ...
  getCreditFromSubscription(from, to) {
    const subscriptionRatios = Subscriptions.getSubscriptionRatio(this._id, from, to);
    let creditMs = 0;
    _.each(subscriptionRatios, function(subscriptionRatio) {
      const plan = Plans.findOne(subscriptionRatio.planId);
      creditMs += subscriptionRatio.ratio * plan.freeCreditMs;
    });
    return creditMs;
  },
  creditMs() {
  ...
  getCurrentPlan() {
    return this.getCurrentSubscription() ? this.getCurrentSubscription().plan() : Plans.NoPlan;
  },
  getRechargedAts(from, to) {
    const subscriptionRatios = Subscriptions.getSubscriptionRatio(this._id, from, to);
    return _.map(subscriptionRatios, function(subscriptionRatio) {
      const plan = Plans.findOne(subscriptionRatio.planId);
      return {
        chargedAt : subscriptionRatio.date,
        amount : subscriptionRatio.ratio * plan.amount,
        planId : plan._id
      }
    });
  },
