...
Customers = {
  ...
  endTask(userId, taskId) {
    let selector = { _id : userId };
    let user = Meteor.users.findOne(selector);
    if (user.currentTask && !user.currentTask.endedAt) {
      let oldCurrentTask = user.currentTask;
      Meteor.users.update(selector,
          { $set:
            { "currentTask":
  ...
  bankTask(userId, taskId) {
    let selector = { _id : userId };
    let user = Meteor.users.findOne(selector);
    if (user.currentTask && user.currentTask.startedAt && user.currentTask.endedAt) {
      Meteor.users.update(selector, { $unset: { "currentTask": "" }});
    } else {
