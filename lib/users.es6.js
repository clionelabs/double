...
Customer = {
  ...
  getRechargedAts(from, to) {
    const subscriptions = Subscriptions.find({customerId : this._id}).fetch();
    let charges = [];
    return _.each(subscriptions, function(subscription) {
      const plan = Plans.findOne(subscription.planId);
      _.each(subscription.rechargedAts, function(rechargedAt) {
        if (rechargedAt > from && rechargedAt <= to) {
          charges.push({
            name : plan.name,
            amount : plan.amount,
