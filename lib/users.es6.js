    ...
    let modifier = { $set :
      {
        ...
        'profile.rebateSecond' : options.profile.rebateSecond
      }
    };
...
Customers = {
  ...
  deductRebateMs : (userId, msToBeDeduct, cb) => {
    let selector = { _id : userId };
    let user = Users.findOneCustomer(selector);
    let resultMs = _.max([ 0, user.profile.rebateMs - msToBeDeduct ]);
    return Meteor.users.update(user._id, { $set : { "profile.rebateMs" : resultMs }}, cb);
  }
};
...
Customer = {
  ...
  rebateMs() {
    return this.profile && this.profile.rebateMs ? this.profile.rebateMs : 0;
  }
};
