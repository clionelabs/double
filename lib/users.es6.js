...
Customers = {
  ...
  deductCreditMs : (userId, msToBeDeduct, from, to, cb) => {
    ...
    msToBeDeduct -= user.creditFromSubscription(from, to);
...
Customer = {
  ...
  creditFromSubscription(from, to) {
    const mySubscriptions = this.subscriptions();

    let creditMs = _.reduce(mySubscriptions, function(memo, subscription) {
      const plan = Plans.findOne(subscription.planId);
      let subscriptionRatios = subscription.ratios(from, to);
      return memo + _.reduce(subscriptionRatios, function(memo, sR) { return memo + sR.ratio; }, 0) * plan.freeCreditMs;
    }, 0);
    return creditMs;
  },
  creditMs() {
  ...
  rechargedAts(from, to) {
    let subscriptions = this.subscriptions();
    let rechargedAts = [];
    _.each(subscriptions, function(subscription) {
      const subscriptionRatios = subscription.ratios(from, to);
