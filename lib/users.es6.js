...
Meteor.users.allow({
  update : function(userId, doc, fields, modifier) {
    let checkIfAllowed = (modifierOps) => {
      let allowedOps = ["$push", "$pull"];
      return _.reduce(modifierOps, function(memo, modOp) {
        return memo && _.contains(allowedOps, modOp);
      }, true);
    };
    ...
    let isSelf = _.isEqual(userId, doc._id);
    let isAllowed = checkIfAllowed(_.keys(modifier));
    return  isUpdatingProfile && (isSelf || assistantIsServingCustomer) && isAllowed;
  }
});
