...
Invoices.Generator = {
  ...
  generateForUsersDue(date) {
    date = moment(date).tz(timezone).startOf('day').valueOf();
    const generator = this;
    const customersSignedUpSameDay = _findCustomersShouldGenerate(date);

    _.each(customersSignedUpSameDay, function(customer) {
      const from = Invoices.findLastBilledDate(customer._id).tz(timezone).startOf('day').add(1, 'day')
                              || moment(customer.createdAt).tz(timezone).startOf('day');
      const to = moment().tz(timezone).startOf('day').valueOf() - 1;
