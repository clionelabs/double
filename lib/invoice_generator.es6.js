...
Invoices.Generator = {
  ...
  generateForUsersDue(date) {
    date = moment(date).tz('Asia/Hongkong').startOf('day').valueOf();
    ...
    _.each(customersSignedUpSameDay, function(customer) {
      ...
      const from = moment(lastBilledDate).tz('Asia/Hongkong').startOf('day').valueOf();
      const to = moment().tz('Asia/Hongkong').startOf('day').valueOf() - 1;
