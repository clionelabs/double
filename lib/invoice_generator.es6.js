...
const _getInvoiceItemsPerDay = (date, task) => {
  ...
  return _.reduce(task.steps, function(memo, step) {
...
const _findCustomersShouldGenerate = (date)=> {
  return _.filter(Users.findCustomers().fetch(), function(customer) {
...
Invoices.Generator = {
  creditFromSubscription : (from, to, userId) => {
    ...
    return _.reduce(mySubscriptions, function(memo, subscription) {
  ...
  rechargedAts : (from, to, userId) => {
    ...
    _.each(subscriptions, function(subscription) {
      ...
      const dateAmountPairs = subscription.chargesFromPeriod(from, to);

      _.each(dateAmountPairs, function(amount) {
        rechargedAts.push(_.extend(amount, {
          planId: plan._id
        }));
      });
    });
  ...
  generate : (from, to, userId) => {
    ...
    _.each(rechargedAts, function (rechargedAt) {
      ...
      const endDate = moment(rechargedAt.periodEndAt).valueOf();
      const dateString = `From ${format(startDate)} to ${format(endDate)}`;
      memberships.push({
        date : rechargedAt.chargedAt,
        amount: rechargedAt.amount,
        title: `${plan.name} | ${dateString}`
      });
    });
    ...
    return _.extend({
      ...
      effectiveRate: customer.minuteRate(),
