...
Invoices.Generator = {
  ...
  generateForUsersDue(date) {
    ...
    _.each(customersSignedUpSameDay, function(customer) {
      const from = Invoices.findLastBilledDate(customer._id).add(1, 'day')
                              || moment(customer.createdAt).tz(customer.timezone()).startOf('day');
      const to = moment().tz(customer.timezone()).startOf('day').valueOf() - 1;
